	...
	public Balade find(int idBalade){
		...
		try {
			...
			if(result.first()) {
				...
				while(result.next()) {
					Vehicule vehicule = daoVehicule.find(result.getString("NUMIMMATRICULATION"));
